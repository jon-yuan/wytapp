...
import com.babuwyt.consignor.MapUtil;
import com.babuwyt.consignor.R;
import com.babuwyt.consignor.adapter.InfoWinAdapter;
import com.babuwyt.consignor.base.BaseActivity;
import com.babuwyt.consignor.bean.WeizhiBean;
...
public class WeiZhiActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        fsendcarno=getIntent().getStringExtra("orderId");
...
//        list.add("D11020170721001");
        ...
        XUtil.GetPing(BaseURL.WEIZHI,list,new MyCallBack<WeizhiBean>(){
            ...
            public void onSuccess(WeizhiBean result) {
                ...
                if (result.isSuccess()){
                    Marker marker=addMarkerToMap(new LatLng(result.getObj().getGps().getWgLat(), result.getObj().getGps().getWgLon()),"","");
                    mapUtil.moveMapCenter(new LatLng(result.getObj().getGps().getWgLat(), result.getObj().getGps().getWgLon()));
                    marker.showInfoWindow();
                    mAdapter.setData(name,result.getObj().getAddress());
                }else {
                    UHelper.showToast(WeiZhiActivity.this,getString(R.string.has_no_data));
                }
